...
function detectPlatform(): "android" | "mac" | "windows" | "linux" {
...
async function fetchReleaseData() {
	...
	const downloads = [
		...
		},
		{
			...
			label: "macOS",
			url: assets.find((a: { name: string }) => a.name.endsWith(".dmg"))
				?.browser_download_url,
		},
		{
			id: "windows",
					...
					<div class={cta}>
						...
						<Show when={platform === "mac"}>
							<small style={{ "margin-top": "0.5rem", display: "block" }}>
								Unsigned build - macOS may warn before opening it.
							</small>
						</Show>
					</div>
				</Motion.div>
